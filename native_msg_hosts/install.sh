# This script is adpated from https://github.com/woodruffw/ff2mpv/blob/master/install.sh
...
PATH_PLACEHOLDER="</path/to/executable>"

# Copying the JSON
if [[ -d "$BROWSER_DEST" ]]; then
    mkdir -p "$JSON_DEST"
    # Replace the placeholder path in the JSON file and install it
    sed -e "s|$PATH_PLACEHOLDER|$CURRENT_DIR/native_msg_hosts/linux/youtube-volume-normalizer.py|g" "native_msg_hosts/$JSON_FILE" >"$JSON_DEST/youtube.volume.normalizer.json"
else
